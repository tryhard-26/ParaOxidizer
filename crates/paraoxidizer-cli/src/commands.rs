...
use paraoxidizer_calibration::{
    recorder::CalibrationEngine, sensitivity::SensitivityEngine, workload::WorkloadProfile,
};
...
use paraoxidizer_optimizer::planner::{OptimizationConstraints, OptimizationPlanner};
...
use paraoxidizer_security::{signature::KeyPair, verification::verify_pox_file};
use std::{
    collections::HashMap,
    fs::File,
    io::{Read, Write},
...
pub fn run_inspect(path_str: &str, format: &str) -> Result<()> {
    ...
    if path.exists() {
        if let Ok(pox) = PoxFile::open(path) {
            ...
            println!(
                "  {}  {}",
                "ParaOxidizer Artifact Inspection:".bold(),
                path_str.green()
            );
            println!(
                "{}\n",
            ...
            println!(
                "  {:<20} {}",
                "Architecture:", pox.metadata.model_config.architecture
            );
            println!(
                "  {:<20} {:.2}B ({})",
                "Parameters:",
                pox.metadata.total_parameters as f64 / 1e9,
                pox.metadata.total_parameters
            );
            println!(
                "  {:<20} {}",
                "Layers:", pox.metadata.model_config.num_hidden_layers
            ...
            println!(
                "  {:<20} {}",
                "Hidden Dimension:", pox.metadata.model_config.hidden_size
            );
            println!(
                "  {:<20} {}",
                "Vocabulary Size:", pox.metadata.model_config.vocab_size
            );
            ...
            println!(
                "  {:<20} {:.1}% ({} tensors)",
                "INT4:",
                (i4_count as f64 / total_t) * 100.0,
                i4_count
            );
            println!(
                "  {:<20} {:.1}% ({} tensors)",
                "INT8:",
                (i8_count as f64 / total_t) * 100.0,
                i8_count
            );
            println!(
                "  {:<20} {:.1}% ({} tensors)",
                "FP16 / Other:",
                (f16_count as f64 / total_t) * 100.0,
                f16_count
            );
    ...
    println!(
        "  {}  {}",
        "Hugging Face / SafeTensors Inspection:".bold(),
        path_str.green()
    );
    println!(
        "{}\n",
    ...
    println!(
        "{}",
        "Architecture & Model Hyperparameters".bold().underline()
    );
    println!(
        "  {:<22} {}",
        "Architecture:", hf_model.model_config.architecture
    );
    println!(
        "  {:<22} {:.2}B params (approx)",
        "Parameter Count:",
        total_weights as f64 / 1e9
    );
    println!("  {:<22} {}", "Tensors Found:", hf_model.tensors.len());
    println!(
        "  {:<22} {}",
        "Hidden Size:", hf_model.model_config.hidden_size
    );
    println!(
        "  {:<22} {}",
        "Intermediate Size:", hf_model.model_config.intermediate_size
    );
    println!(
        "  {:<22} {}",
        "Hidden Layers:", hf_model.model_config.num_hidden_layers
    );
    println!(
        "  {:<22} {}",
        "Attention Heads:", hf_model.model_config.num_attention_heads
    );
    println!(
        "  {:<22} {}",
        "KV Heads (GQA/MQA):", hf_model.model_config.num_key_value_heads
    );
    println!(
        "  {:<22} {}",
        "Vocab Size:", hf_model.model_config.vocab_size
    );
    ...
    println!(
        "  {:<22} {:.2} GB (Baseline)",
        "FP16 (Uncompressed):", fp16_gb
    );
    println!(
        "  {:<22} {:.2} GB (~2.0x reduction)",
        "INT8 Symmetric:", int8_gb
    );
    println!(
        "  {:<22} {:.2} GB (~3.6x reduction)",
        "INT4 Group-128:", int4_gb
    );
    println!(
        "  {:<22} Supported (Llama, Qwen, Mistral, Gemma, Phi)",
        "Compatibility:"
    );
...
pub fn run_hardware(format: &str) -> Result<()> {
    ...
    println!(
        "  {:<24} {} physical, {} logical",
        "Cores:", hw.physical_cores, hw.logical_cores
    );
    println!(
        "  {:<24} {:.1} GB (Available: {:.1} GB)",
        "System Memory (RAM):",
        hw.total_ram_mb as f64 / 1024.0,
        hw.available_ram_mb as f64 / 1024.0
    );
    ...
    println!(
        "  {:<24} {}",
        "Apple Silicon Metal:",
        if hw.has_apple_silicon_gpu {
            "YES (Unified Memory Architecture)".green()
        } else {
            "NO".dimmed()
        }
    );
    println!(
        ...
        "CUDA Support:",
        if hw.has_cuda_gpu {
            "YES (NVIDIA Accelerator detected)".green()
        } else {
            "NO".dimmed()
        }
    );
    ...
    println!(
        "  {:<24} {}",
        "Optimal Format:",
        hw.recommended_format().yellow().bold()
    );
    println!(
        "  {:<24} {}",
        "Target Backend:",
        hw.recommended_runtime_backend().cyan().bold()
    );
...
pub fn run_calibrate(
    ...
    println!(
        "Calibrating with workload profile: {}",
        profile.to_string().cyan().bold()
    );
    ...
    println!(
        "Processing {} activation traces across {} tensors...",
        samples,
        tensor_names.len()
    );
    ...
    println!(
        "{}",
        format!(
            "Successfully generated calibration artifact: {}",
            output_path
        )
        .green()
        .bold()
...
pub fn run_analyze(model_path: &str, calibration_path: Option<&str>, format: &str) -> Result<()> {
    ...
    for t in report.tensors.iter().take(25) {
        let level_cell = match t.level {
            paraoxidizer_calibration::sensitivity::SensitivityLevel::Critical => {
                Cell::new(t.level.to_string()).fg(Color::Red)
            }
            paraoxidizer_calibration::sensitivity::SensitivityLevel::High => {
                Cell::new(t.level.to_string()).fg(Color::Yellow)
            }
            paraoxidizer_calibration::sensitivity::SensitivityLevel::Medium => {
                Cell::new(t.level.to_string()).fg(Color::Blue)
            }
            paraoxidizer_calibration::sensitivity::SensitivityLevel::Low => {
                Cell::new(t.level.to_string()).fg(Color::Green)
            }
        };
        table.add_row(Row::from(vec![
            ...
            Cell::new(format!(
                "{} (g{})",
                t.recommended_precision, t.recommended_group_size
            )),
        ]));
    }
    ...
    if report.tensors.len() > 25 {
        println!(
            "... and {} more tensors analyzed.",
            report.tensors.len() - 25
        );
    }
...
pub fn run_quantize(
    ...
    let metadata = PoxMetadata {
        ...
        base_model_name: Path::new(model_path)
            .file_name()
            .unwrap_or_default()
            .to_string_lossy()
            .to_string(),
    ...
    println!(
        "Quantizing {} tensors to INT{} using [{}] (group size: {})...",
    ...
    for (name, (shape, _, mut floats)) in hf_model.tensors {
        ...
        if bits == 4 {
            let (packed, scales) = match algorithm.to_lowercase().as_str() {
                "awq" => {
                    let cols = if shape.dims().len() >= 2 {
                        shape.dims()[1]
                    } else {
                        shape.numel()
                    };
                    let rows = if shape.dims().len() >= 2 {
                        shape.dims()[0]
                    } else {
                        1
                    };
    ...
    println!(
        "{}",
        format!(
            "Successfully generated quantized .pox artifact: {}",
            output_path
        )
        .green()
        .bold()
...
pub fn run_optimize(
    ...
    println!(
        "\n{}",
        "=== ParaOxidizer Adaptive Mixed-Precision Optimizer ==="
            .cyan()
            .bold()
    );
    ...
    if format == "json" {
        ...
        println!(
            "\nOptimal Plan Configuration: {}",
            plan.selected_point.name.green().bold()
        );
        ...
        println!(
            "  Estimated Quality: {:.1}%",
            plan.selected_point.quality_pct
        );
    }
    ...
    println!(
        "\nCompiling hardware-optimized .pox artifact to {}...",
        output_path
    );
    for (name, (shape, _, mut floats)) in hf_model.tensors {
        let target_prec = plan
            .layer_precisions
            .get(&name)
            .cloned()
            .unwrap_or_else(|| "INT4".into());
        ...
        if target_prec == "FP16" {
            ...
            // INT4
            let outliers =
                SparseOutlierTable::extract_and_zero_outliers(&mut floats, outlier_policy);
            let outlier_bytes = outliers.as_ref().map(|o| o.to_bytes());
    ...
    println!(
        "{}",
        format!(
            "Successfully generated optimal .pox artifact: {}",
            output_path
        )
        .green()
        .bold()
...
pub fn run_validate(model_path: &str) -> Result<()> {
    ...
    println!(
        "Checking header magic and version... OK (POX v{})",
        pox.header.format_version
    );
    println!(
        "Checking tensor descriptors... {} tensors found",
        pox.tensors.len()
    );
    ...
    println!(
        "{}",
        "Artifact validated: Numerical validity, offsets, and tensor consistency VERIFIED."
            .green()
            .bold()
    );
    Ok(())
...
pub fn run_verify(model_path: &str, trusted_pubkey: Option<&str>) -> Result<()> {
    ...
    if report.is_trusted(false) {
        ...
        return Err(PoxError::Security(
            "Artifact failed cryptographic verification".into(),
        ));
    }
...
pub fn run_benchmark(
    ...
    if let Some(model_path) = model_path {
        ...
        return Err(paraoxidizer_core::error::PoxError::Runtime(
            "Please provide a model path or pass --suite to execute the hardware benchmark suite."
                .into(),
        ));
    }
...
pub fn run_compare(models: &[String]) -> Result<()> {
    ...
    println!(
        "\n{}",
        "=== ParaOxidizer Multi-Artifact Comparison ==="
            .cyan()
            .bold()
    );
    println!("{}", table);
...
pub fn run_sign(model_path: &str, key_hex: &str, output_path: Option<&str>) -> Result<()> {
    ...
    println!(
        "{}",
        format!("Successfully signed and saved artifact: {}", out_file)
            .green()
            .bold()
    );
    Ok(())
...
pub fn run_inspect_run(run_id_or_path: &str) -> Result<()> {
    ...
    let pox = if path.exists() {
        ...
        return Err(PoxError::Format(format!(
            "Run record '{}' not found. Please pass path to .pox file.",
            run_id_or_path
        )));
    };
    ...
    println!(
        "  Source Model Hash:   {}",
        pox.manifest.source_model_sha256
    );
    println!(
        "  Calibration Hash:    {}",
        pox.manifest.calibration_sha256.as_deref().unwrap_or("None")
    );
    ...
    println!(
        "  Tensors Manifest:    {} verified entries",
        pox.manifest.tensor_hashes.len()
    );
...
pub fn run_reproduce(run_id_or_path: &str) -> Result<()> {
    ...
    println!(
        "{}",
        "REPRODUCIBILITY CHECK: 100% Deterministic Match ✓"
            .green()
            .bold()
    );
    Ok(())
...
pub fn run_diff(model_a_path: &str, model_b_path: &str) -> Result<()> {
    ...
    println!(
        "Model A (Baseline):  {} ({:.1} MB)",
        model_a_path,
        size_a as f64 / (1024.0 * 1024.0)
    );
    println!(
        "Model B (Candidate): {} ({:.1} MB)",
        model_b_path,
        size_b as f64 / (1024.0 * 1024.0)
    );
    let ratio = if size_b > 0 {
        size_a as f64 / size_b as f64
    } else {
        1.0
    };
    println!(
        "Size Differential:   {:.2}x reduction ({:.1} MB saved)\n",
        ratio,
        (size_a as f64 - size_b as f64) / (1024.0 * 1024.0)
    );
...
pub fn run_build(config_path: &str) -> Result<()> {
    ...
    let mem_limit = cfg
        .optimization
        .as_ref()
        .and_then(|o| o.memory_limit.as_deref());
    let lat_limit = cfg
        .optimization
        .as_ref()
        .and_then(|o| o.latency_limit.as_deref());
...
pub fn run_keygen(output_prefix: &str) -> Result<()> {
    ...
    println!(
        "{}",
        "Generated new Ed25519 signing keypair:".green().bold()
    );
