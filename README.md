...
### 3. Full-Graph Generative Evaluation & Inference Performance (Perplexity, KL-Divergence, TTFT & Throughput)

To evaluate real-world generative performance beyond individual matrix cosine similarities, ParaOxidizer executes an end-to-end decoder-only Transformer computational graph—including **Rotary Position Embeddings (RoPE)**, **Grouped-Query Attention (GQA)**, **Causal KV-Caching**, **Post-Attention RMSNorm**, and **SwiGLU Feed-Forward Blocks (`gate * silu(up) -> down`)**.

Evaluated on **Apple Silicon M4** (10-core ARM NEON, unified memory) using `cargo run --release -p paraoxidizer-bench --bin bench-model-performance`:
